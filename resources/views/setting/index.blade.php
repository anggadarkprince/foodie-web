...
@section('content')
    <form action="{{ route('admin.settings') }}" method="post" id="form-setting">
        @csrf
        @method('put')
                        ...
                        <div class="relative w-full">
                            <select class="form-input pr-8 disabled:opacity-75 disabled:text-gray-500" name="default-management-group" id="default-management-group">
                                ...
                                @foreach($groups as $group)
                                    <option value="{{ $group->id }}"{{ old('default-management-group', app_setting('default-management-group')) == $group->id ? ' selected' : '' }}>
                                        {{ $group->group }}
                                    </option>
                                @endforeach
                            </select>
